...
/**
 * MCP tool {@code replace_field} — replaces the child at {@code key} in a CEDAR template
 ...
 * replaced may be a field or an element — the slot keeps its key either way.
 */
public final class ReplaceFieldTool
{
  ...
  public static McpSchema.Tool tool()
  {
    ...
    properties.put("child", Map.of(
        "type", "string",
        "description",
        "Replacement CEDAR field as YAML — the kind of artifact 'create_field' returns. "
    ...
    properties.put("name", Map.of(
        ...
        "Optional property label override for the parent's _ui block. If omitted, the "
            + "replacement field's own schema:name is used."));
    properties.put("description", Map.of(
        "type", "string",
        "description",
        "Optional property description override for the parent's _ui block. If omitted, "
            + "the replacement field's own schema:description is used."));
    ...
    properties.put("isRequired", Map.of(
        ...
        "Whether a value for this field is required in instances of the parent (the field's "
            + "requiredValue constraint). Optional; when omitted, whatever the replacement "
            + "child already carries is preserved. Applies to the copy embedded in THIS "
            + "parent — the standalone child artifact is unchanged."));
    properties.put("isHidden", Map.of(
        "type", "boolean",
        "description",
        "Whether the field is hidden in forms rendered from the parent (the field's _ui "
            + "hidden flag). Optional; when omitted, whatever the replacement child already "
            + "carries is preserved. Applies to the copy embedded in THIS parent."));
    ...
    return McpSchema.Tool.builder()
        .name("replace_field")
        .title("Replace a child of a template or element with a CEDAR field")
        .description(
            "Replaces the child at 'key' in a CEDAR template or element with the supplied "
                + "field, keeping the key and its position in the display order (remove_child "
                + "+ add_field would move it to the end). isRequired / isHidden optionally set "
                + "the embedded copy's required-value constraint and hidden flag for this "
                + "parent. Returns the updated parent as expanded YAML, re-validated with "
                + "CedarValidator."
      ...
      McpSyncServerExchange exchange, McpSchema.CallToolRequest request)
  {
    ...
    try {
      ...
      // Tri-state: only override when supplied, so replacing a field never silently strips
      // a required/hidden setting the replacement child already carries.
    ...
    try {
      ...
      return error("replace_field failed: " + e.getClass().getSimpleName() + ": " + e.getMessage());
    }
