...
public final class CreateTemplateInstanceTool
{
  ...
  public static McpSchema.Tool tool()
  {
    ...
    properties.put("id", Map.of(
        "type", "string",
        "description",
        "IRI that identifies the instance itself (the @id). Optional, and normally omitted: "
            + "CEDAR mints an artifact's identifier when it is created on a server, so an instance "
            + "built here carries none until then. Supply one only to repeat an id a CEDAR "
            + "repository already assigned. Must be an absolute IRI. Distinct from isBasedOn, "
            + "which names the template rather than the instance."));
